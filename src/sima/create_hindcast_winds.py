...
def __create_hindcast(hc_name, hc_values):
    ...
    hindcast.latitude = hc_values["longitude_actual"]
    hindcast.longitude = hc_values["latitude_actual"]
    hindcast.wind = winds
    return hindcast
...
if __name__ == "__main__":
    ...
    epoch = values["time"]
    dstart = datetime.fromtimestamp(epoch[0])
    dend = datetime.fromtimestamp(epoch[-1])
    sd = dstart.strftime("%Y.%m.%d.%H")
    ed = dend.strftime("%Y.%m.%d.%H")
    filename = f"{name}_{sd}-{ed}"
    path = Path(f"./output/simamet/{filename}.h5")
